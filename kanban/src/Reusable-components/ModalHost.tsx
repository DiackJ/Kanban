...
import ResetBoardModal from "../Modals/ResetBoardModal.tsx";
import AddColumnModal from "../Modals/AddColumn.tsx";
import type { GetBoardNav } from "../Types/GetNavBoards.tsx";

function ModalHost({modal, open, openClose, board, setBoard, boards}: {modal: string; open: boolean; openClose: () => void; board: GetBoard | null; setBoard: (id:number) => void; boards: GetBoardNav[]}) {
            ...
            {open && (
                <div className="flex justify-center items-center px-2 fixed inset-0 bg-black/60">
                    <motion.div 
                            ...
                            className={`border rounded-lg h-fit w-[390px]
                        ${theme === "light"
                        ...
                        <div
                            ...
                            {(() => {
                                switch(modal) {
                                    case "createBoard":
                                        return <CreateBoardModal close={openClose}/>
                                    case "createTask":
                                        return <CreateTaskModal board={board} close={openClose}/>
                                    case "createColumn":
                                        return <AddColumnModal board={board} close={openClose}/>
                                    case "editBoard":
                                        return <EditBoardModal board={board} close={openClose}/>
                                    case "deleteBoard":
                                        return <DeleteModal type="board" board={board} close={openClose} setBoard={setBoard} boards={boards}/>
                                    case "resetBoard":
                                         return <ResetBoardModal board={board} close={openClose}/>
                                    default:
                                        return null
                                }}
                            )()}
